...
test=test
prod=prod
git-repo=https://github.com/jbrannst/appdev-foundations-kitchensink
date
...
oc process -f cicd-template.yaml -p TEST_PROJECT=${test} -p PROD_PROJECT=${prod} -p GIT_REPO=${git-repo} | oc create -f - -n ${cicd}
